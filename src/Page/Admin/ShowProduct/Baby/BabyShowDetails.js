...
const BabyShowDetails = ({babyProduct}) => {
     const {_id,name,price,image,details,quentity}=babyProduct;
       
    const handleDelete =_id =>{
        console.log(_id)
        fetch (`https://my-app-server-liard.vercel.app/babyCosmetics/${_id}`,{
            method:'DELETE'
        })
    ...
    <Link  to={`/babyEdit/${_id}`} >   <button className="h-12 w-28  mt-4 bg-white text-pink-950 font-bold border-4 border-pink-950 ml-4 mb-2">✏️ 𝐔𝐩𝐝𝐚𝐭𝐞  </button></Link> 
...
export default BabyShowDetails;