...
const AllUsers = () => {
  ...
  const { data: users = [], refetch } = useQuery({
    queryKey: ['users'],
    queryFn: async () => {
      const res = await axiosSecure.get('/users', {   // ✅ Changed URL to serverless route
        headers: {
  ...
  const handleMakeAdmin = (user) => {
    axiosSecure.patch(`/users/admin/${user._id}`)   // ✅ Changed URL
      .then((res) => {
  ...
  const handleDelete = (user) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure.delete(`/users/${user._id}`)  // ✅ Changed URL
          .then((res) => {
  ...
  return (
    <div>
      <div className="flex justify-evenly my-4">
        <h2 className="text-xl font-serif text-white">𝐀𝐥𝐥 𝐔𝐬𝐞𝐫𝐬</h2>
        <h2 className="text-xl font-serif text-white">𝐓𝐨𝐭𝐚𝐥 𝐔𝐬𝐞𝐫𝐬: {users.length}</h2>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="table table-zebra w-full">
          <thead>
            <tr>
              <th className="text-xl font-bold text-white font-serif"></th>
              <th className="text-xl font-bold text-white font-serif">Name</th>
              <th className="text-xl font-bold text-white font-serif">Email</th>
              <th className="text-xl font-bold text-white font-serif">Role</th>
              <th className="text-xl font-bold text-white font-serif">Action</th>
            </tr>
          </thead>
          <tbody>
            {users.map((user, i) => (
              <tr key={user._id}>
                ...
                <td>
                  <button
                    ...
                    title="Delete User"
                  >
                    <AiFillDelete className="text-red-700 mr-2" />
                  </button>
                </td>
              </tr>
            ))}
